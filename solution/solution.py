...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "Rubric:\n"
            f"{rubric_lines}\n"
            "Return JSON mapping criterion names to normalized scores between 0 and 1."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            first_scores.append(values[0])

        overall_average = sum(all_scores) / len(all_scores) if all_scores else 0.0
        first_average = sum(first_scores) / len(first_scores) if first_scores else 0.0

        return {
            "positional_bias": bool(first_scores) and first_average > 0.8,
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def _avg(items: list[EvalResult], attr: str) -> float:
            if not items:
                return 0.0
            return sum(getattr(item, attr) for item in items) / len(items)
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = suggestions[index - 1] if index - 1 < len(suggestions) else "Investigate failure pattern"
            root_cause = self.find_root_cause(failure)
            lines.append(
                f"| F{index:03d} | {failure.failure_type or 'unknown'} | {root_cause} | {suggestion} | Open |"
            )
        return "\n".join(lines)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append("Implement stricter grounding checks and improve retrieval quality before generation")
        if categories.get("irrelevant"):
            suggestions.append("Refine prompts and routing so answers stay tightly aligned to the user question")
        if categories.get("incomplete"):
            suggestions.append("Increase retrieved evidence coverage and add examples of fully complete answers")
        if categories.get("off_topic"):
            suggestions.append("Add intent detection and topic validation before finalizing responses")
        if categories.get("refusal"):
            suggestions.append("Review safety rules to reduce unnecessary refusals on allowed questions")

        generic_suggestions = [
            "Expand the golden dataset with more edge cases from recent failures",
            "Add regression checks in CI/CD so metric drops block risky releases",
            "Review the lowest-scoring benchmark cases weekly and cluster them by root cause",
        ]
